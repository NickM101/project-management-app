...
registerForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (profilePicInput.files?.[0]) {
      ...
      formData.append("name", userName.trim()); // Backend expects 'name', not 'userName'
      formData.append("email", email.trim());
      formData.append("password", password);
      formData.append("role", "USER"); // Add default role
      ...
      const jsonPayload = {
        ...
        // Note: confirmPassword is not sent to backend - it's frontend validation only
      };
